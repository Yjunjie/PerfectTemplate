...
/// 操作数据库的基类
class BaseOperator {
    let dataBaseName = "perfect_notea"
    var mysql: MySQL {
        get {
    ...
    func queryUserInfo(userName: String, password: String) -> String? {
        ...
        if !mysql.query(statement: statement) {
            ...
            if results.numRows() == 0 {
                ...
                print("[\(#line)]->\(statement)用户名或密码错误，请重新输入")
            } else {
                results.forEachRow { row in
                    guard let userId = row.first! else {//保存选项表的Name名称字段，应该是所在行的第一列，所以是row[0].
                        return
...
class ContentOperator: BaseOperator {
    ...
    ///   - content: 内容
    /// - Returns: 返回结果JSON
    func addContent(userId: String, title: String, content: String,create_time: String) -> String? {
        let values = "('\(userId)', '\(title)', '\(content)','\(create_time)')"
        let statement = "insert into \(contentTableName) (userID, title, content,create_time) values \(values)"
        print("[\(#line)]->执行SQL:\(statement)")
    ...
    func queryContentList(userId: String) -> String? {
        ...
        if !mysql.query(statement: statement) {
            self.responseJson[ResultKey] = RequestResultFaile
            self.responseJson[ErrorMessageKey] = "查询失败"
            print("[\(#line)]->SQL:\(statement)查询失败")
        } else {
            print("[\(#line)]->SQL:\(statement)查询成功")
            
            // 在当前会话过程中保存查询结果
            let results = mysql.storeResults()! //因为上一步已经验证查询是成功的，因此这里我们认为结果记录集可以强制转换为期望的数据结果。当然您如果需要也可以用if-let来调整这一段代码。
            print("[\(#line)]->results==\(results)")
            var ary = [[String:String]]() //创建一个字典数组用于存储结果
            if results.numRows() == 0 {
                print("[\(#line)]->\(statement)尚没有录入新的Note, 请添加！")
            } else {
                results.forEachRow { row in
...
//                    }
        ...
        if !mysql.query(statement: statement) {
            ...
            if results.numRows() == 0 {
                ...
                results.forEachRow { row in
                    ...
                    dic["content"] = "\(content)"
                    print("[\(#line)]->row:\(row)==content==\(content)")
                }
 ...
 /// 更新内容
    ...
    ///   - content: 内容
    /// - Returns: 返回结果JSON
    func updateContent(contentId: String, title: String, content: String,create_time: String) -> String? {
        
 

        let statement = "update \(contentTableName) set title='\(title)', content='\(content)', create_time='\(create_time)' where id='\(contentId)'"
        print("[\(#line)]->执行SQL:\(statement)")
        
        if !mysql.query(statement: statement) {
            self.responseJson[ResultKey] = RequestResultFaile
            self.responseJson[ErrorMessageKey] = "更新失败"
            print("[\(#line)]->\(statement)更新失败")
        } else {
            print("[\(#line)]->SQL:\(statement) 更新成功")
            self.responseJson[ResultKey] = RequestResultSuccess
        }
    ...
    func deleteContent(contentId: String) -> String? {
        ...
        if !mysql.query(statement: statement) {
            ...
            print("[\(#line)]->\(statement)删除失败")
        } else {
            print("[\(#line)]->SQL:\(statement) 删除成功")
            self.responseJson[ResultKey] = RequestResultSuccess
        }
